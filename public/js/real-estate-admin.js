...
$(window).on("load", function () {
  ...
  $(".city_id").on("change", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        $("#city_area_id").empty();
        $("#city_area_id").html(
          "<option selected value> Select city area... </option>",
        );
  ...
  $(".info-area-icon").after(
    '<div class="infoTitle d-none"></div> <i class="showInfoArea ml-2 active fas fa-info-circle" style="cursor: pointer" title=""></i> ',
  );
  ...
  function agent_list() {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#dropdown-menu-admin").append(
          '<input type="text" placeholder="Search.." class="form-control" id="myInputadmin" onKeyUp="filterFunction()">',
        );
        $("#dropdown-menu-checklist").append(
          '<input type="text" placeholder="Search.." class="form-control" id="myInputchecklist" onKeyUp="filterFunctionCheck()">',
        );
            ...
            if (value.rating) {
              $("#dropdown-menu-admin").append(
                '<li class="dropdown-item" data-agent-id="' +
                  ...
                  '" width="50px" height="50px" class="br-100 v-mid mr-2">  ' +
                  value.first_name +
                  " " +
                  value.last_name +
                  " (" +
              ...
              $("#dropdown-menu-admin").append(
                '<li class="dropdown-item"  data-agent-id="' +
                  ...
                  '"><img src="' +
                  value.img_src.encoded +
                  '" width="50" height="50"  class="br-100 v-mid mr-2">  ' +
                  value.first_name +
                  " " +
                  value.last_name +
                  "</li>",
  ...
  if (property_id == "") {
    ...
    $(".p-subcategory").each(function () {
      ...
      if ($(this).attr("data-id") == category_id) {
        ...
        $(this).html(
          '<span class="category-tick-selected"><i class="fas fa-check"></i></span> ' +
            $(this).attr("data-category_name"),
        );
    ...
    liObj = $("#dropdown-menu-admin").find(
      "li[data-agent-id=" + agent_id + "]",
    );
    var btnObj = liObj.parent().siblings("button");
    $(btnObj).text(liObj.text());
    ////popup agent
    liObj = $("#dropdown-menu-checklist").find(
      "li[data-agent-id=" + agent_id + "]",
    );
    var btnObj = liObj.parent().siblings("button");
    $(btnObj).text(liObj.text());
...
$(document).ready(function () {
  ...
  $(document).on("click", '[data-id="project"]', function () {
    ...
    $('[data-id="project"]').html(
      '<span class="tick-selected"><i class="fas fa-check"></i></span> INVEST',
    );
  ...
  $(".category-parent-active").click(function () {
    $(".p-category")
      .removeClass("category-parent-active")
      .addClass("category-parent-inactive");
  ...
  $(".type_sale").click(function () {
    ...
    $(this).removeClass("label-secondary").addClass("label-primary");
    $(this).html(
      '<span class="tick-selected"><i class="fas fa-check"></i></span> ' +
        $(this).attr("data-type-name"),
    );
          ...
          $.each(response.data.documents, function (key, value) {
            ...
            if (property_id == "" && value.documents.is_delete == 0) {
              if (response.data.document_images) {
                ...
                doc_image =
                  '<br><a target="_blank" class="thisisthetest" href="/storage/' +
                  path +
                  '"><img src="/storage/' +
                  path +
                  '"  class="image-box-wrapper mb-2" width="50%" style="height:50%"></a><input type="hidden" data-src="/storage/' +
                  path +
                  '">';
              }
              ...
              $(".verify-documents").append(
                '<div class="row">' +
                  '<div class="col">' +
                  '<input type="checkbox"  class="checklist"  data-document-id="' +
                  value.document_id +
                  '" name="verify_document[]" value="' +
                  value.document_id +
                  '"/><span> I have verified the ' +
                  value.documents.name +
                  " </span>" +
                  "</div>" +
                  "</div>",
              ...
              if (response.data.document_images) {
                ...
                /*if(id==value.id)*/
                doc_image =
                  '<br><a target="_blank" href="/storage/' +
                  path +
                  '">' +
                  "See Document" +
                  '</a><input type="hidden" data-src="/storage/' +
                  path +
                  '">';
              }
              /*if(response.data.document_images)
  ...
  if (user && property_id != "") {
    $.ajax({
      ...
      success: function (response) {
        ...
        let subcategory = $("ul.sub-category li.label-primary")
          .attr("data-category_name")
          .trim();
          ...
          success: function (response) {
            if (response.status) {
              if (response.approved && author_id != "") {
                toastr.success(
                  "Document Checklist Updated successfully! You can approve the property.",
                  "success",
                  {
                ...
                toastr.success(
                  "Document Checklist Updated successfully! You can approve the property after verifying all the documents.",
                  "success",
                  {
                    iconClass: "toast-custom",
                    timeOut: 10000,
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.status) {
              ...
              toastr.success("Agent Assigned successfully!", "success", {
                iconClass: "toast-custom",
              });
  ...
  $("#admin_comment").click(function (e) {
    ...
    if (comment != "") {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if ($(".d-comment").find(".display-comment").length) {
            ...
            $(".display-comment:last").append(
              '<img src="' +
                data.admin_url.encoded +
                '" class="br-100 v-mid mr-2" style="width: 30px;"><strong>' +
                data.admin.first_name +
                '</strong><p class="comment-desp">' +
                data.comment +
                "</p>",
            );
          } else {
            ...
            $(".comment-box")
              .find(".d-comment")
              .append(
                '<div class="display-comment"><img src="' +
                  data.admin_url.encoded +
                  '" class="br-100 v-mid mr-2" style="width: 30px;"><strong>' +
                  data.admin.first_name +
                  '</strong><p class="comment-desp">' +
      ...
      $("input[name='comment']")
        .parent()
        .after(
          "<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter comment</div>",
        );
    }
  });
