            ...
            @if ($agents->count())
                @foreach($agents as $key => $val)
                    <div class="col-md-6">
                        <figure>
                            @if($val->image_path)
                            <img src="storage/{{ $val->image_path }}" style="border: 1px solid #fff;border-radius: 50%;" alt="Image">
                            @else
                            <img src="{{ $val->avatar_url }}" alt="Image">
                            @endif
                            <figcaption>
                                <h4> <a href="{{ route('public.agent.detail', $val->username) }}"><span>{{$val->first_name}}</span>
                                        {{$val->last_name}} </a></h4>
                                ...
                                <ul>
                                    ...
                                    @if($val->phone != "")
                                        ...
                                        <button type="button" class="showContact btn btn-primary pt-1 pb-1"
                                            data-id="{{$val->id}}">Show Contact</button>
                                    @endif
