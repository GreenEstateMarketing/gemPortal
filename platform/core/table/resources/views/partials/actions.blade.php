...
<div class="table-actions">
    ...
    @if(Route::current()->getName()=="property.index" &&  auth()->user()->super_user==1)
        ...
        <a href="{{ route('consult.property.consults',$item->id) }}" class="btn btn-icon btn-sm btn-primary" title="Consults"><i class="fas fa-headset"></i><span class="badge badge-info" id="consult_count">{{ auth()->user()->getConsults($item->id)}}</span></a>
    @endif
    @if (!empty($edit))
